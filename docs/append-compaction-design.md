...
- `player_agent.compaction.max_prompt_tokens` (optional): hard cap on the same estimate, independent of context length. Needed because a provider profile replaces `context_token_limit` with the model's own context (1M on the current frontier endpoints), which puts the fraction trigger out of reach. Set well above a normal segment (200k in config-5.0 against a largest measured 20-turn segment of 105k) so it stays a safety valve; the interval remains the rule because the 2026-09-07 cost fit found per-turn cost within 8% of each model's optimum at 20 turns even though the optimum itself ranges from ~10 turns / 22k tokens (gpt-6-astra, $1/M cached) to ~20 turns / 100k tokens (gemini-3.8-flash, $0.075/M cached). Reason on the compaction event: `token_cap`.
...
- `player_agent.transport.max_retries` / `compaction.max_retries`: bounded re-asks for OUTPUT defects only (wrong tool, unparsable JSON, schema miss, truncated completion) — immediate, few. Provider-side failures never spend this budget.
- An EMPTY `inputs` list is an output defect too (2026-09-11). It is checked in code after schema validation, not as `minItems` on the schema, so the tool JSON schema — part of the wire contract every continue re-verifies — is unchanged. The first retry after an empty answer carries one appended user note (`EMPTY_INPUTS_NOTE`: at least one input per turn; progress is judged from game memory, an open message box means the event is not yet processed; close dialogue with A/B). Why: gpt-6-astra(low) beat Brock, read "received the BOULDERBADGE" off the battle screen and returned `[]` for 80 turns; the post-battle script that sets the badge flag never ran, and the leg cap ended a run that had won.
- A TEXT reply in tool mode — anything but exactly one call to the phase's tool — is `MissingToolCall` (2026-09-12; the message stays "Expected exactly one <phase> call"). The first retry carries one appended user note (`tool_call_note(phase)`: the reply was not a tool call, text outside a tool call is never read, answer with exactly one `<phase>` call). Before this the retry was byte-identical to the failed request. Why: deepseek-v4.1-flash(max) answered the handover prompt as a JSON string, a fenced block or `<continuation_summary>` tags on 6 of 12 compaction attempts under `tool_choice: auto`; at turn 110 all three attempts did and the run crashed. For compaction the accepted history is discarded by `compact()`, so the note never reaches a later request; in gameplay it stays in the conversation like the empty-inputs note.
- EVERY other output defect (unparsable tool arguments, a schema miss, `finish_reason: length`, an oversized handover) also gets a note on the retry (2026-09-12, `output_defect_note(phase, mode, exc)`): the error text the harness saw, collapsed to one line and capped at 600 chars, then the required shape ("exactly one `<phase>` tool call whose arguments match that tool's schema", or the JSON-object wording in prompted mode), plus "make it shorter" when the completion was cut off. One note per distinct error text; a repeated identical error does not stack notes. Andreas: "in the retry feed the model with the error message of what it has to do".
- Profile `tool_choice: named` forces the phase's own tool on each request. The wire-contract digest reads a dict naming the phase's tool as the policy `"named"`, not the value, so gameplay and compaction requests share one contract (before 2026-09-12 the first handover under `named` raised ContinuityError). `compaction.max_retries` is 4 in config-5.1 (was 2): a failed compaction ends the run, and retries now differ. The deepseek-v4.1-flash profile used `named` for one day: the first-party `deepseek` route advertises tool_choice but OpenRouter's tool-compatibility filter rejects `named` and `required` with HTTP 404 (probed 2026-09-12 evening; only `auto` is accepted), so the fresh max run died at turn 1 and the profile is back on the default `auto`, relying on the retry note. `named` remains available for endpoints that honour it.
- Class docstrings are popped from the wire schemas (2026-09-12). pydantic copies a model's docstring into `model_json_schema()["description"]`, so PlayAction's 2026-09-08 docstring ("…schema quirks (Muse's stringified null, mimo's "None")…") reached every request, as the tool's `parameters.description` in tool mode and inside the prompted schema message. Anthropic's classifier refused claude-fable-5.1(medium) on it (`finish_reason: content_filter`, native `refusal`: "restrictions on reverse engineering or duplicating model outputs") on all three attempts of turn 1; a standalone replay was refused again and the same request with the descriptions stripped was answered. Field descriptions are kept. This changes the tools schema, so the wire-contract digest of runs started under config-5.1 before the change no longer matches: those runs cannot be continued, only restarted. The prompted instruction also gained "Include no keys beyond those the schema lists": claude-opus-5(high) returned a valid action plus one invented key (predicted_screen, memory, prediction) three times in a row; the generic retry note fixes that on the next attempt (probe 2026-09-12: valid PlayAction), the sentence avoids paying for the retry.
- Prompted output drops an extra key whose value is null before validation (`drop_null_extras`, src/agent/coerce.py, 2026-09-12 evening). claude-fable-5.1(medium) padded turn 1 with `"inputs_note": null`, then `"result": null`, then a trailing comma, and lost all three gameplay attempts; Fable cannot leave prompted mode because Anthropic's classifier refuses its requests with `response_format` (3/3) and with `tools` (3/3, same day). A null under an unread key carries nothing, so the drop is lossless; a non-null extra key still fails `extra="forbid"` and the retry note names it, and tool / native_json output is untouched (schema-enforced on the wire). `transport.max_retries` in config-5.1 is 4 (was 2): with the error quoted on each retry, a model whose defect changes per attempt gets a different correction each time, and a well-formed model never pays for the headroom. JSON syntax is still never repaired.
- A reply json.loads rejects is raised as `MalformedJSON` whose message comes from `describe_json_error` (2026-09-12 evening). json's own message points at a character offset, useless when the offset is simply the end of the reply: qwen3.8-flash(thinking) ended every compaction reply one `}` short (turns 21, 41, 61, 101), the retry quoted "Expecting ',' delimiter: line 1 column 6318 (char 6317)" back five times at turn 101, and the run died. When the text ends with brackets still open the note now says how many and which closers are missing ("missing `}` at the very end"); an unterminated string is named as such; a defect in the middle keeps json's message. Still no repair: the model closes its own object on the retry.
- Profile `close_unbalanced_json` (default false; true for qwen/qwen3.8-flash, Andreas 2026-09-12 evening, option "a"): when a PROMPTED reply fails to parse, ends outside a string, and has brackets still open, the derived closers are appended and the completed text parsed; success is recorded as an `output_coerced` event (`kind: closed_brackets`, `appended`, `chars`) so the trace shows the repair. Anything else wrong with the text — a trailing comma, an unterminated string, extra data — still raises the original `MalformedJSON` and takes the retry path; tool and native_json output are untouched. This is the one deliberate exception to "never repair JSON": Qwen's turn-101 handover was one `}` short on 7 of 7 attempts, two of them after a note naming the missing closer, and appending it reproduced the turn-81 memory structure key for key — the note approach demonstrably cannot fix a systematic one-closer defect, and the flag scopes the repair to the model where it is proven.
- Profile `strip_leading_prose` (default false; true for anthropic/claude-fable-5.1, 2026-09-13): the second and only other allowed prompted repair, same gate (`AppendAgent._repair_prompted`). When a PROMPTED reply fails to parse and is exactly "a prose prefix, then one complete JSON object", the object is parsed and the prose recorded as `output_coerced` (`kind: stripped_prose`, `prose`, `chars`). Text after the object, or an object that does not parse, still raises the original `MalformedJSON`. Independently of the flag, `describe_json_error` now names the prose ("The reply began with prose ('Prediction correct: ...') instead of the JSON object") instead of quoting offset 0. Anchor: claude-fable-5.1(medium) run 2026-09-13_15-50-38 cleared turns 1-7 with the null-key drop in place, then wrote "Prediction correct: Oak's event triggered..." before the object on 5 of 5 attempts at turn 8 and crashed; the object after the prose was complete every time.
- claude-opus-5 (and sonnet-5, copied) moved to `output_mode: native_json` the same day, and `native_json` now sends `strict: true` on the json_schema. Reason: even with the sentence, the fresh 14:09 Opus run invented a key on 2 of 3 first attempts (`prediction`, then a garbled `"}": null`), each a paid retry. Live probes of the same requests with `response_format` json_schema strict: 3 of 3 gameplay and 1 of 1 handover validated, thinking still returned on gameplay. claude-fable-5.1 stays prompted: with a `response_format` on the request Anthropic's classifier refused 3 of 3 (`content_filter`, "duplicating model outputs"), while prompted mode without the docstring answered 3 of 3.
- `native_json` types the handover `memory` as a JSON string (2026-09-12, same day): under strict json_schema the provider closes every object, so the open memory object came back as `{}` on all three compactions of the first Opus run while the summary was full; a string memory came back with 10 keys. Handover's before-validator decodes it. The opus-5 / sonnet-5 profiles also carry `memory_encoding: json_string`.
- **Contract rebase on a continue** (2026-09-12): `QueuedRun.rebase_contract` (continue-only; `pokemon runs continue <run> --rebase-contract`; `rebase_contract: true` in the continue body). The executor sets `cfg["_rebase_contract"]` and re-resolves the provider profile from the current catalog; the agent then (a) accepts a changed profile on `restore()` and (b) accepts ONE wire-contract digest change at the first request, each as a `contract_rebased` event (`stage: profile | wire`, `changed: [...]` — per-field digests are now kept in `state["wire_contract_fields"]` so the wire event can name the fields; a state that predates them reports unknown). `state["contract_rebases"]` keeps the list. A second change in the same segment is still a `ContinuityError`, and without the flag nothing changed: a continue whose profile or wire differs is refused. Why: the memory fix above had to reach a run 77 turns in (Andreas: "can't we stop it, restart and continue?"); the default stays strict because a silent mid-run change makes a score incomparable — this one is loud.
- `player_agent.transport.transient_retries`, `backoff_base_seconds`, `backoff_factor`, `backoff_cap_seconds`, `retry_budget_seconds` (2026-09-09, `src/agent/backoff.py`): the TRANSIENT budget for 429 / 5xx / dropped connections / timeouts / truncated bodies. The identical request is re-sent after equal-jitter exponential backoff (ceilings 5, 10, 20, 40, 80, 160, 320, then 600 s in config-5.x; the wait is the upper half of the ceiling), honouring `Retry-After` / `X-RateLimit-Reset` as a floor, until either the attempt count or the wall-clock budget (45 min) runs out. Each wait is an `llm_backoff` event (terminal line + live-feed box); exhaustion emits `kind: exhausted` and the original error propagates. 400/401/402/403/404/405/422 and continuity violations stay fatal on the first failure. Motivation: a qwen3.8-flash run crashed at T21 on upstream 429s retried three times five seconds apart.
