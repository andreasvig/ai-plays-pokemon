...
`configs/provider-profiles.yaml` contains 21 model/endpoint profiles — one per active registry model after the 2026-09-07 prune (43 entries → 21 active + 2 retired) — plus five named Gemma variants. `config-5.0.yaml` (the append harness, renamed from the experimental `config-append.yaml` and now the default for casual AND official runs) opts in; the legacy configs (`config-4.0`, `config-3.13`) and existing saved runs keep their previous behavior. Resolved profiles are saved inside run config and conversation checkpoints. Resume rejects a changed profile before sending another request. An UNPROFILED model no longer keeps the pre-profile transport: as of 2026-09-07 the `defaults:` block applies to every model, so it gets `transforms: []`, the cache-mode contract, `final_turn_text_only` and the provider/model-drift guard. What it does not get is a fabricated endpoint — `endpoint` stays empty (unpinned) and its own registry `provider:` block, if any, still routes it. Being profiled is not a certification either way; it records what one endpoint was probed to do.
