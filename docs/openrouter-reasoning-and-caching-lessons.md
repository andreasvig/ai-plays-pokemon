...
| Implicit caching, markers hurt (2) | Google AI Studio: on real 27k+ prompts, no marker → 89% (gemini-3.8-flash) / 60% (3.5-flash-lite) cached from the second send on, extension included; any explicit marker suppresses it and caches only the marked block. Nothing hits below ~16k prompt tokens in ANY shape (15k sent three times identically: 0); Google documents a 4,096 minimum, so the floor is this route's. Above it the live shape extends; a text-only final turn (the OpenAI fix) made extension MISS at 27k and 43k, so Gemini keeps final_turn_text_only false. The 2026-09-06 verdict ("implicit never fires") came from prompts below the floor. |
