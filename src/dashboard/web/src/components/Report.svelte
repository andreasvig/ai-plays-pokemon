<script>
  ...
  // Per-input census — rendered by the shared InputCensus component, which the
  // public model page uses too, so the two surfaces cannot drift.
...
<style>
  ...
  /* Where the inputs went — same card shell as the scorecard; the bars, the
     labels and the footnote live in InputCensus.svelte. */
