 ...
 * The cable's colour.
 *
 * A full turn of the colour wheel every `COLOUR_LOOP_TILES` tiles WALKED, not
 * once across the whole run (Andreas, 2026-09-15: "make the colour loop shorter
 * also as a configurable"). One ramp stretched over a 2,266-tile run made a
 * town the run crossed in ten minutes a single flat colour; looping it gives
 * local contrast back — you can tell one pass down a corridor from the next.
 *
 * What it costs: the colour no longer says WHEN in absolute terms, because the
 * same green comes round every 300 tiles. What it still says, and says better,
 * is HOW FAR — equal colour distance is equal tiles walked, in every run, so
 * two runs are directly comparable by eye. The legend says exactly that.
 *
 * A wheel, not a blue→red ramp, because a ramp has to get home somehow: cut it
 * and every loop boundary is a false edge, mirror it and each colour means two
 * different distances. A hue wheel closes on itself and does neither.
 */
 ...
 * first-badge region that is the two Route 2 gate-side maps — is drawn beside
 * the world. A map the atlas flags `popup` is not laid out at all: it lives in
 * its building's popup (M10), which since 2026-09-16 includes Viridian Forest
 * and the two gate houses that open onto it, stacked as one place.
 */
export function worldLayout(route, atlas) {
  ...
  const entries = Object.keys(route.maps)
    ...
    // `popup`, not `type === 'MAP_TYPE_INDOOR'`: since 2026-09-16 Viridian
    // Forest — a ROUTE — is drawn inside its gate houses' popup, so the world
    // frame asks the atlas which maps live in a popup rather than inferring it
    // from the map type (index version 2).
    .filter(([, m]) => m && !m.popup)
  ...
  return {
    ...
    interiors: Object.keys(route.maps).filter((k) => atlas.maps[k]?.popup),
  }
}
 ...
 * was won. A battle on a map this layout does not draw is dropped — on the
 * world frame that is every fight indoors, and they appear when the map walks
 * into the building they happened in.
 */
export function battlesFor(layout, route) {
  const out = []
  for (const [i, b] of (route?.battles ?? []).entries()) {
    if (!Array.isArray(b.tile)) continue
    const p = layout?.at[`${b.tile[0]}:${b.tile[1]}`]
    if (!p) continue
