# SkyEmu start states, one per game

Each game's opening: the player standing in a room, with control, everything
...
**Each directory here is a savepoint dir** — `emulator.state` + `state.json` +
`tasks.json` (`src/app/starts.py:REQUIRED_FILES`), plus a `preview.png` a human
looks at once. That is the same shape a run's own savepoint has, which is why the
executor resumes one of these through exactly the code path a `--continue` uses.

Wired in two registries, both of which point at the directory rather than the
file: `configs/roms.yaml` (`start_save`, the game's default opening) and
`configs/starts.yaml` (the same directory again, under a name the picker and
`pokemon queue add --start` can ask for). Deliberately one directory for both, so
the default opening and the named opening cannot drift apart.

`emulator.state` and `preview.png` are written by
`v2-experiments/make_start_states.py` (and, for Emerald,
`make_emerald_state.py`). **Regenerate rather than edit** — the scripts are the
source of truth and they verify their own output. `state.json` and `tasks.json`
are authored, not measured, and the generator leaves them alone: regenerating a
game refreshes its state without touching its goal text.

| game | console | where | cost |
|---|---|---|---|
| crystal | GB\* | the bedroom upstairs in New Bark Town | 9,156 frames, 7 s |
| emerald | GBA | the moving van in Littleroot | 17,172 frames, 34 s |
...
\* Crystal is a GBC cartridge and the registry calls it `GB`, because `GB` is
what `src/emulator/backends/frame.py` MEASURES from a 160x144 capture. The
vocabulary is the measured one on purpose, so a declared console and an observed
one compare directly instead of through a mapping table.

`firered-pokebench-v2/` is FireRed's and predates the rest — it is the one a
benchmark already runs from, written by `v2-experiments/make_start_state.py`
(singular). Emerald's `probe` state, which the address finder uses because the
van is too cramped to measure movement in, stays in
`v2-experiments/states/emerald/`: it is an experiment fixture, not an opening.

## Why these are replayed and not converted

Everything in `configs/saves/` OUTSIDE this directory is an **mGBA** savestate
and SkyEmu answers `/load -> failed` for all of them. There is no converter, so the opening is
...
also produces the Littleroot house state (`probe`, kept in
`v2-experiments/states/emerald/`), and why that is the one the address finder
uses.
