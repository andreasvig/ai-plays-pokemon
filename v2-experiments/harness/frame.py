"""What the model is shown — and the two things v1 did that v2 must not.

**No grid overlay.** ``src/emulator/emulator.py:_draw_grid_overlay`` paints a
semi-transparent red lattice every 16 native px with an 8 px vertical offset.
...
def prepare(png: bytes, upscale: Optional[int] = None,
    ...
    Returns (png bytes, meta) where meta carries the system, the final size and
...
def image_row_to_touch_y(row: int, meta: dict) -> float:
    ...
    The inverse of the trap in emulator-research.md: touch_x/touch_y are
    normalised over the touch SCREEN, while anything read off the frame is in
    the coordinates of the stacked IMAGE. Every conversion goes through here.
    """
