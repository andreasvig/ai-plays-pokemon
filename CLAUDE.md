...
tests/
  ...
  test_movement.py       # Movement calibration tests
...
- **Memory dictionary**: Agent's persistent memory across turns. Stored as JSON state file. Agent writes updates via `memory_updates` string field on `GameAction` output (not a tool call). Harness parses JSON and applies updates after each turn. Required keys: location, party, goal, story_progress, map, obstacles.
- **Coordinate system**: Player at (0,0). x: left=negative, right=positive. y: up=negative, down=positive. Ranges for uncertainty: `(-3..-4, 2..3)`.
