...
- **Socket protocol**: Newline-delimited commands over TCP port 8888. `CAP` for screenshots, `SEQ:btn1;btn2` for button sequences. Sequences are fire-and-forget (sleep for calculated duration, no TCP recv wait). A/B buttons use a longer gap (`ab_gap_frames`, default 45) than directional buttons (`frames_between_inputs`, default 24) to allow dialogue to render.
- **Screen stability**: After button execution, captures 3 frames at `poll_interval` apart (120×80 grayscale), compares all 3 pairwise. Stable when avg similarity ≥ threshold (starts 0.99, relaxes to 0.95 over `max_wait`).
